...
def test_run_soda_checks_supports_partitioned_report_dir(monkeypatch, tmp_path: Path) -> None:
    from crypto_belief_pipeline.dq import soda as soda_mod
    ...
    reports_dir = Path("reports/date=2026-05-07/hour=12")
    out = soda_mod.run_soda_checks("2026-05-07", db_path=dbp, reports_dir=reports_dir)
    assert out["report_paths"] == {
        "output_txt": str(reports_dir / "soda_scan_output.txt"),
        "summary_json": str(reports_dir / "soda_scan_summary.json"),
    }
