...
- **Bounded price as-of joins can still miss labels**: `features/prices.py` uses bounded as-of joins to compute `t-1h`, `t-4h`, `t+1h`, `t+4h`, `t+24h` close prices. If candles are missing outside the configured tolerance, labels remain null by design rather than attaching stale prices.
